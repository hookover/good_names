...
class Process extends Command
{
     ...
     * @var string
     */
    protected $signature = 'names:process';
...
//        $middlename = KangxiModel::where('char', '=', '弥')->first();
//        $lastname = KangxiModel::where('char', '=', '澈')->first();
    ...
    public function runing()
    {
        ...
        foreach ($this->mu as $mu) {
            foreach ($this->shui as $shui) {
                $middlename = KangxiModel::where('char', '=', trim($mu))->first();
                $lastname = KangxiModel::where('char', '=', trim($shui))->first();

                if(!isset($middlename->char)) {
                    $this->info('错误:' . $mu);
                    continue;
                }
                if(!isset($lastname->char)) {
                    print_r($lastname);
                    $this->info('错误:' . $shui);
                    continue;
                }
                ...
                $this->info('陈' . $middlename->char . $lastname->char . " && 陈" . $lastname->char . $middlename->char);
